...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['archive'])) {
    ...
    $stmt->execute();
    header('Location: students.php'); // Redirect to refresh page
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    ...
    $sql = "DELETE FROM student_info WHERE User_ID = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $user_id);
    $stmt->execute();

    // Delete from student_details
    $sql = "DELETE FROM student_details WHERE User_ID = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_student'])) {
    ...
    $role = 'student'; // Role is 'student' for this case
    $student_number = $_POST['student_number']; // Student number from the form
    ...
    $sql_user = "INSERT INTO tbl_users (email, password, role, student_number) 
                 VALUES ('$email', '$password_hash', '$role', '$student_number')";

    if ($conn->query($sql_user) === TRUE) {
        // Step 2: Get the User_ID of the newly inserted user
        $user_id = $conn->insert_id; // Get the last inserted User_ID
        
        // Step 3: Now insert into student_details
        $sql_student_details = "INSERT INTO student_details (User_ID, first_name, last_name, year_level, course) 
                             VALUES ('$user_id', '$first_name', '$last_name', '$year_level', '$course')";
        
        if ($conn->query($sql_student_details) === TRUE) {
            echo "New student account created successfully!";
        } else {
            echo "Error inserting into student_details: " . $conn->error;
        }
    } else {
...
// Fetch student data (with student details)
$sql = "SELECT 
            u.User_ID, 
            u.email, 
            u.student_number, 
...
<body>
    ...
    <h1 class="text-center mb-4" style="color: #004E98; font-weight: bold;">STUDENT INFORMATION</h1>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addAccountModalLabel">Add New Student</h5>
                ...
                <div class="modal-body">
                    <form method="POST" action="students.php">
                        ...
                        <button type="submit" name="add_student" class="btn btn-add-account">Add Student</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <tbody>
            <?php while ($student = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        <!-- View -->
                        <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#viewProfileModal<?= $student['User_ID']; ?>">View</button>
                ...
                <!-- View Modal -->
                <div class="modal fade" id="viewProfileModal<?= $student['User_ID']; ?>" tabindex="-1" aria-labelledby="viewProfileModalLabel" aria-hidden="true">
