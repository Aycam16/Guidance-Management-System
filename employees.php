...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['archive'])) {
    ...
    if ($stmt) {
        ...
        if (!$stmt->execute()) {
            error_log("Error archiving employee: " . $stmt->error);
        }
    }
    header('Location: ' . filter_var('employees.php', FILTER_SANITIZE_URL));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    ...
    $sql = "DELETE FROM employee_info WHERE User_ID = ?";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("i", $user_id);
        if (!$stmt->execute()) {
            error_log("Error deleting from employee_info: " . $stmt->error);
        }
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_employee'])) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            $sql_employee_info = "INSERT INTO employee_info (User_ID, name, department) VALUES (?, ?, ?)";
            $stmt_info = $conn->prepare($sql_employee_info);
            if ($stmt_info) {
                $stmt_info->bind_param("iss", $user_id, $name, $department);
                if ($stmt_info->execute()) {
                    $_SESSION['success_message'] = "New employee account created successfully!";
                } else {
                    error_log("Error inserting into employee_info: " . $stmt_info->error);
                }
            }
        } else {
...
// Fetch Employee Data with Search Functionality
$search = $_GET['search'] ?? '';
$sql = "SELECT u.User_ID, u.email, ei.name, ei.department 
        FROM tbl_users u
        JOIN employee_info ei ON u.User_ID = ei.User_ID
        WHERE u.role = 'employee' AND (ei.name LIKE ? OR ei.department LIKE ? OR u.email LIKE ?)";
$stmt = $conn->prepare($sql);
if ($stmt) {
    $searchTerm = "%$search%";
...
<body>
    ...
    <!-- HTML for Employee Management -->
<div class="container mt-4">
    <h1 class="text-center mb-4" style="color: #004E98; font-weight: bold;">EMPLOYEE INFORMATION</h1>
            ...
            <div class="modal-content">
                <form method="POST" action="employees.php">
                    ...
                    <div class="modal-footer">
                        <button type="submit" name="add_employee" class="btn btn-add-account">Add Employee</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <tbody>
            <?php while ($employee = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        <!-- View Modal -->
                        <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#viewProfileModal<?= $employee['User_ID']; ?>">View</button>

                        <!-- Archive -->
                        <form method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to archive this employee?');">
                ...
                <!-- View Profile Modal -->
                <div class="modal fade" id="viewProfileModal<?= $employee['User_ID']; ?>" tabindex="-1">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5>View Profile</h5>
